...
export const liveJapao = {
  ...
  comunidade: {
    ...
    url: 'https://chat.whatsapp.com/DiLcpTRVhSALsAlnx5NDbf',
   ...
   * Cada live escolhe o que pede. Não é preferência de layout: é o que a
   * automação consegue fazer depois.
   *
   *   pedirWhatsapp → habilita o disparo do ManyChat em cima da lista de
   *                   inscritos e é a chave que cruza "inscritos" com
   *                   "presentes" (a lista do /entrar). Sem ele, não há
   *                   findbycomm no Bitrix e cada inscrito vira contato novo.
   *   pedirEmail    → é o que permite ao n8n criar o convite no Google Agenda
   *                   com a pessoa como convidada.
   *
   * Pelo menos UM dos dois precisa ser true: o servidor recusa lead sem
   * nenhuma forma de contato.
   */
  formulario: {
    pedirWhatsapp: true,
    pedirEmail: false,
...
/**
 * Formato de uma live. Serve de contrato para as outras (live-peru.ts): se um
 * campo faltar ou mudar de tipo lá, o `tsc -b` do build acusa aqui.
 */
export type ConfigLive = typeof liveJapao
