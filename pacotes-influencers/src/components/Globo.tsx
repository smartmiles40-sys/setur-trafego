...
import { ESCALA, expedicoes } from '../data/expedicoes'
...
// ver lib/jornada.ts).
//  • Parada no topo: a Terra gira inteira com TODAS as expedições da STFV
//    acesas pelo mundo (autoridade).
//  • Começou a rolar: as expedições saem e ficam só as rotas dos 5 pacotes;
//    depois a câmera voa de destino em destino desenhando a linha da viagem.

const B = import.meta.env.BASE_URL
const FIM_EXPEDICOES = 0.5 // até aqui (telas roladas) as expedições aparecem

type Arco = {
  id: string
  tipo: 'expedicao' | 'trilho' | 'luz' | 'viagem'
  cor: string
  startLat: number
  startLng: number
  endLat: number
...
function criarRotulo(texto: string, cor: string, pequeno = false) {
  const el = document.createElement('div')
  el.className = pequeno ? 'rotulo-globo rotulo-expedicao' : 'rotulo-globo'
  el.innerHTML = `<i style="background:${cor}"></i>${texto}`
  return el
}

export default function Globo({ u }: { u: MotionValue<number> }) {
  const globo = useRef<GlobeMethods | undefined>(undefined)
  const caixa = useRef<HTMLDivElement>(null)
  const [tam, setTam] = useState({ w: 0, h: 0 })
  const [mostraExpedicoes, setMostraExpedicoes] = useState(true)
  ...
  // Linhas das expedições: diretas de São Paulo, ou com escala em Doha pra Ásia.
  // A altura do arco acompanha a distância (perto = baixinho, longe = mais alto).
  const arcosExpedicoes = useMemo<Arco[]>(() => {
    ...
    return [
      ...
      ...expedicoes.map((e) => linha(`e-${e.nome}`, e.escala ? ESCALA : ORIGEM, e)),
    ]
  }, [])
  ...
  const pontosExpedicoes = useMemo<Ponto[]>(
    () => [...expedicoes.map((e) => ({ lat: e.lat, lng: e.lng, cor: '#D7F264', raio: 0.32 })), { ...ESCALA, cor: '#ffffff', raio: 0.18 }],
    [],
  )
  const pontosDestinos = useMemo<Ponto[]>(() => destinos.map((d) => ({ lat: d.lat, lng: d.lng, cor: d.cor, raio: 0.4 })), [destinos])

  const rotulosExpedicoes = useMemo<Rotulo[]>(
    () => expedicoes.map((e) => ({ id: e.nome, lat: e.lat, lng: e.lng, el: criarRotulo(e.nome, '#D7F264', true) })),
    [],
  )
  const rotulosDestinos = useMemo<Rotulo[]>(() => destinos.map((d) => ({ id: d.slug, lat: d.lat, lng: d.lng, el: criarRotulo(d.rotulo, d.cor) })), [destinos])

  const arcos = useMemo(
    () => (mostraExpedicoes ? arcosExpedicoes : [...saidas.slice(0, acesas * 2), ...viagem.slice(0, trechos)]),
    [mostraExpedicoes, arcosExpedicoes, saidas, viagem, acesas, trechos],
  )
  const pontos = useMemo(
    () => [pontoOrigem, ...(mostraExpedicoes ? pontosExpedicoes : pontosDestinos.slice(0, acesas))],
    [mostraExpedicoes, pontoOrigem, pontosExpedicoes, pontosDestinos, acesas],
  )
  const rotulos = useMemo(() => {
    if (modoRotulos === 'exp') return rotulosExpedicoes
  ...
  }, [modoRotulos, rotulosExpedicoes, rotulosDestinos, acesas])
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (e !== ultimo.e) setMostraExpedicoes((ultimo.e = e))
  ...
  const pronto = (g: GlobeMethods | undefined) => {
    ...
    g.renderer().setPixelRatio(Math.min(window.devicePixelRatio, toque ? 1.5 : 2))
    const c = g.controls()
    c.enableZoom = false
    ...
    <div ref={caixa} className="pointer-events-none absolute inset-0">
      {tam.w > 0 && (
        <Globe
          ...
          arcColor={(d: object) => {
            const a = d as Arco
            if (a.tipo === 'expedicao') return ['rgba(215,242,100,0.7)', 'rgba(255,255,255,0.85)']
            if (a.tipo === 'trilho') return ['rgba(215,242,100,0.35)', `${a.cor}66`]
            return a.tipo === 'viagem' ? ['#D7F264', '#ffffff'] : ['#D7F264', a.cor]
          }}
          arcStroke={(d: object) => ({ expedicao: 0.45, trilho: 0.35, luz: 0.9, viagem: 1.1 })[(d as Arco).tipo]}
          // Rotas longas (Ásia) ficam baixas pra não "saltar" pra fora da Terra.
          arcAltitude={(d: object) => (d as Arco).altura ?? null}
          arcAltitudeAutoScale={0.42}
          arcDashLength={(d: object) => ({ expedicao: 0.4, trilho: 1, luz: 0.28, viagem: 1 })[(d as Arco).tipo]}
          arcDashGap={(d: object) => ({ expedicao: 0.25, trilho: 0, luz: 0.9, viagem: 0 })[(d as Arco).tipo]}
          arcDashAnimateTime={(d: object) => ({ expedicao: 3200, trilho: 0, luz: 2600, viagem: 0 })[(d as Arco).tipo]}
